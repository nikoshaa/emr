...
<script>
        ...
        fetch(`/chat/users?search=${encodeURIComponent(searchTerm)}&page=${page}`)
            ...
            .then(response => {
                ...
                users.forEach(user => {
                    ...
                    html += `
                    <div class="user-item d-flex align-items-center p-3 border-bottom" data-user-id="${user.id}">
                        <div class="avatar avatar-sm mr-3 ">
    ...
    function renderMessages(messages) {
        ...
        messages.forEach(message => {
            const isFromStaff = !message.is_staff_message;
            const messageClass = isFromStaff ? 'staff-message' : 'user-message';
            const alignClass = isFromStaff ? 'justify-content-start' : 'justify-content-end';
            const bubbleClass = isFromStaff ? 'bg-light text-dark' : 'bg-primary text-white';
            ...
            <div class="message-item d-flex ${alignClass} mb-3">
                <div class="message-content ${messageClass} ${bubbleClass}" style="max-width: 75%; padding: 10px 15px; border-radius: 15px;">
                    ...
                    <div class="message-time text-right">
                        <small class="${isFromStaff ? 'text-muted' : 'text-white-50'}">
    ...
    function subscribeToUserChannel(userId) {
        ...
        const channelName = `chat-channel-${userId}-${currentStaffRole}`;
    ...
    function handleNewMessage(data) {
        ...
        if (data.message) {
            ...
            addMessageToChat(data.message, data.is_staff_message);
        }
    ...
    function addMessageToChat(message, isFromStaff) {
        console.log("IS FROM STAFFF ", isFromStaff)
        const chatMessages = document.getElementById('chat-messages');
        const alignClass = !isFromStaff ? 'justify-content-start' : 'justify-content-end';
        const bubbleClass = !isFromStaff ? 'bg-light text-dark' : 'bg-primary text-white';
        const messageClass = !isFromStaff ? 'staff-message' : 'user-message';
